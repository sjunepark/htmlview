...
| Annotation state and feedback are durable and bounded                   | Permission/schema/recovery/limit tests; atomic send/end/discard; stop/delete failure barriers; cursor retry, cancellation, restart, and tombstones |
| Human browser feedback reaches the foreground agent                     | Playwright element/freeform queue, blocking CLI delivery, cursor acknowledgement, automatic refresh, explicit-discard End, and closure             |
...
- Every ready review owns one non-persistent fixed-entry watcher, tracks at
  ...
  and aborts polling without consuming that budget, pageshow resumes it, and a
  successful local End terminates immediately. Review-scope closure cancels
  watcher, timers, and managed inspection fibers.
